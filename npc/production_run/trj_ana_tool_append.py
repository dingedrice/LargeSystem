'''
This file is to calculate RMSD, Q, and COM of the NPC system, and append the results to existing files.
...
#    sim_init = sys.argv[2]
    ...
    print("start trj loading")
    dcd_path = f'{sim_dic}/npc_trajectory.dcd' 
    trj = mda.Universe(reffile, dcd_path)        
    print("trj loaded")

    with open(f"{work_dir}/contact_DB_uniq_c.pkl", "rb") as f:
    ...
    with open(f"{sim_dic}/rmsd.pkl", "rb") as f:
        rmsd_all = pickle.load(f)
    print("rmsd loaded")

    qi_all = list(np.load(f"{sim_dic}/qi_DB_c.npy"))
    print("qi loaded")

    com_chains_all = list(np.load(f"{sim_dic}/chain_COM_fit2c.npy"))
    print("com loaded")

    rmsd_regions_all = {region: list(rmsd_all.rmsd_regions[region]) for region in rmsd_all.rmsd_regions}
    rmsd_chains_all = list(rmsd_all.rmsd_chains)
#    qi_all = []
#    com_chains_all = []
